TITLE Mod file for component: Component(id=sca type=ionChannelHH)
...
PARAMETER {
    ...
    h_instances = 1 
    h_forwardRate_rate = 1.5240001E-4 (kHz)
    h_forwardRate_midpoint = -13 (mV)
    h_forwardRate_scale = -50 (mV)
    h_reverseRate_rate = 0.00217 (kHz)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=sca type=ionChannelHH), from conductanceScaling; Component(id=null type=q10ConductanceScaling)
    ...
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=sca type=ionChannelHH), from gates; Component(id=m type=gateHHrates)
